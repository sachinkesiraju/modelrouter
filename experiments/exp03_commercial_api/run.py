"""Routing analysis over commercial OpenAI models with measured $ costs.

Reads results/openai_scores.json (from score_openai.py) and evaluates
always-X, oracle, and prompt-embedding-router policies using each model's
measured average cost per request.
"""
...
def main() -> None:
    blob = json.loads((HERE / "results" / "openai_scores.json").read_text())
    models = blob["models"]
